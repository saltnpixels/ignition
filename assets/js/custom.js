...
jQuery(function ($) {
    ...
    var ToggleEvent = new Event('afterToggle');
    ...
    function moveItems() {
        ...
        $moveItems.forEach(function (item) {
            ...
            if (!source) {
                var sourceElem = item.parentElement.id;
...
var iconAngleRight = "<span class='arrow'></span>";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    //if the menu button is outside site-top. get both buttons
...
.not('[href="#"]').not('[href="#0"]').click(function (event) {
    ...
    if (location.pathname.replace(/^\//, '') == this.pathname.replace(/^\//, '') && location.hostname == this.hostname) {
        ...
        var offset = $(this.hash).data('offset');
