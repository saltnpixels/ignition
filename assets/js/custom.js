...
jQuery(function ($) {
    ...
    // Set the active submenu to be toggled on if its in the panel (mobile) or its display none, which only applies to vertical menus
    ...
    if (currentSubmenus.css('display') === 'none' || currentSubmenus.parents('#panel-left, #panel-right').length) { //submenus are set to display none only in vertical menus which is what we want
        //add toggled on to the li and the button
        console.log((currentSubmenus.siblings('.menu-item-link').find('.submenu-dropdown-toggle')));
        currentSubmenus.siblings('.menu-item-link').find('.submenu-dropdown-toggle').trigger('click');
